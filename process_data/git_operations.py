...
def get_github_user(github_client: Github) -> str:
    ...
    while True:
        try:
            ...
            if long_retries > 3:
                raise exc
            LOGGER.warning("Failed to connect to GitHub. Retrying in 10 seconds.")
            sleep(10)
...
def commit_push_pr(github_pat: str) -> None:
    ...
    if len(list(prs)) == 0:
        ...
        LOGGER.info("Created a pull request to merge your latest changes.")
    else:
        LOGGER.info("New changes pushed to an existing pull request.")
...
def synchronize_github(github_pat: str) -> None:
    ...
    if str(repo.active_branch) == "main":
        ...
        try:
            ...
            LOGGER.warning("Remote branch does not exist.")
            if repo.is_dirty():
                LOGGER.warning("Repository is dirty. Attempting to merge `main`.")
                repo.git.pull("main")
                commit_push_pr(github_pat)
            else:
                LOGGER.info(
                    "Repository is clean. Checking out `main` and deleting existing branch."
                )
